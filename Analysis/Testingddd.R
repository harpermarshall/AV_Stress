...
# GOOD IND PARTICIPANTS: 03, kinda 05, 12, 15 kinda, 16 kinda, 17 (PERF), 
rt_abs_min <- 0
rt_abs_max <- 10000
drop_incorrect <- TRUE
drop_outliers <- FALSE
...
make_plot_delta_cdf <- function(clean_trials_df) {
  ...
  lab_pos_text <- lab_pos %>%
    ...
    mutate(
      x_label = max(bins) * 0.60,   # 60% across panel
      y_label = max(diff, na.rm = TRUE) * 0.85  # near top
    )
...
run_race_model_permutation_test <- function(clean_trials_df, n_perm = 5000) {
  ...
  for (soa in offsets) {
    ...
    # compute participant-level violation curves
    diff_mat <- matrix(NA, nrow = length(participants), ncol = length(bins))
    ...
    for (i in seq_along(participants)) {
      ...
      ecdf_A   <- ecdf(dat_p$rt[dat_p$modality == "A"])
      ecdf_V   <- ecdf(dat_p$rt[dat_p$modality == "V"])
      ecdf_AVC <- ecdf(dat_p$rt[dat_p$modality == "AVC"])
      
      if (soa < 0) {
        
        Miller <- pmin(ecdf_A(bins) + ecdf_V(bins + soa), 1)
        
      } else if (soa > 0) {
        
        Miller <- pmin(ecdf_A(bins - soa) + ecdf_V(bins), 1)
    ...
    for (p in 1:n_perm) {
      ...
      signs <- sample(c(-1, 1), size = nrow(diff_mat), replace = TRUE)
...
perm_results <- run_race_model_permutation_test(df_plot, n_perm = 5000)

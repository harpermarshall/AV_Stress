...
### AVC, AVI, V MEDIANS OVERLAYED ###
# Step 1: Filter to AVI, V, AVC and compute per-participant medians
median_rt_overlay_avi <- stroop_clean %>%
  filter(Type %in% c("AVI", "V", "AVC")) %>%
  group_by(Participant, VisualOffset, Type) %>%
  summarise(median_rt = median(RT * 1000, na.rm = TRUE), .groups = "drop")

# Step 2: Assign participant colors
participants <- unique(median_rt_overlay_avi$Participant)
participant_colors <- scales::hue_pal()(length(participants))
names(participant_colors) <- participants

# Step 3: Plot
ggplot(median_rt_overlay_avi, aes(x = VisualOffset, y = median_rt, group = interaction(Participant, Type))) +
  geom_line(aes(color = Participant, linetype = Type), linewidth = 1, alpha = 0.8) +
  geom_point(aes(color = Participant, shape = Type), size = 2) +
  scale_color_manual(values = participant_colors) +
  scale_shape_manual(values = c("AVC" = 17, "AVI" = 15, "V" = 16)) +  # triangle, square, circle
  scale_linetype_manual(values = c("AVC" = "solid", "AVI" = "dashed", "V" = "dotted")) +
  scale_y_continuous(name = "Median RT (ms)", breaks = seq(200, 600, 50)) +
  coord_cartesian(ylim = c(200, 600)) +
  theme_minimal() +
  labs(
    title = "Overlayed Median RTs for AVI, V, and AVC Trials by Participant",
    x = "Visual Offset (ms)",
    color = "Participant",
    linetype = "Trial Type",
    shape = "Trial Type"
